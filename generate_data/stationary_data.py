...
def symmetric_oscillatory(X, A1, A2, period1, period2, sigma=0.1):  # 0.5
    Y = A1 * np.sin((2 * np.pi * X) / period1) + A2 * np.sin((2 * np.pi * X) / period2) + np.random.normal(0, sigma,
                                                                                                           len(X))
    return Y
...
def generate_data(time_span, num_of_period, step, replicates, data_type, param, data_name='', period1=24, period2=24,
                  A1=3, A2=3, noise=0.1, population_id=1, df=pd.DataFrame(), repetition=0):
    if data_name == '':
        data_name = data_type
    ...
    for ix in range(replicates):
        if data_type == 'symmetric_non_oscillatory':
            ...
            data_params = {'time_span': time_span, 'num_of_period': num_of_period, 'step': step,
                           'replicates': replicates, 'noise': noise}

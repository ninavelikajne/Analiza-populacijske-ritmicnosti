...
from fit import helpers as hlp, dproc, plotter
import pandas as pd
...
plotter.subplot_model(X_test, Y_1, X_test, Y_1, ax, plot_model=True, plot_measurements=False, fit_label='Symmetric Oscillatory', color='red')
plotter.subplot_model(X_test, Y_2, X_test, Y_2, ax, plot_model=True, plot_measurements=False, fit_label='Asymmetric Oscillatory 1', color='blue')
plotter.subplot_model(X_test, Y_3, X_test, Y_3, ax, plot_model=True, plot_measurements=False, fit_label='Asymmetric Oscillatory 2', color='green')
ax.legend()
plt.savefig('../results/gee/gen_data.png')
plt.show()
...
og_df=pd.read_csv('../results/sep_classification_gee.csv')
...
for index, row in df.iterrows():
    ...
    if row['amplitude']<0.1: # no rhythm
        ...
        if row['data_name']=='symmetric_oscillatory':
            ...
            # fig, ax = plt.subplots(1, 1, figsize=(12, 7))
            # plot_cosopt.subplot_model(X_test, Y_ss, X_test, Y_ss, ax, plot_model=True,plot_measurements=False,fit_label='yss',color='green')
            # plot_cosopt.subplot_model(X_test, Y_test, X_test, Y_test, ax, plot_model=True,plot_measurements=False,fit_label='fit',color='blue')
            # ax.legend()
            # plt.show()
    ...
    temp=row[['method','data_name','data_params','time_span','num_of_period','step','replicates','noise','population_id','Y_test','repetition','changing_param']]
...
df_results.to_csv('../results/rmse_gee.csv',index=False)

...
def calculate_confidence_intervals_parameters_cosinor(df, predict_var, time_var, group_var, variables=None,interactions=None,repetitions=20, period=24, n_components=1, plot=True, save_to=''):
    ...
    for i in range(0, repetitions):
        ...
        df_result = gee_cosinor(sample, predict_var, time_var, group_var, interactions=interactions,variables=variables,n_components=n_components, period=period, plot=False)
...
def gee_cosinor(df, predict_var, time_var, group_var, variables=None,interactions=None, n_components=1,period=24, plot=True,save_to='',plot_title='',summary=False):
    df_results=pd.DataFrame()
    if plot:
    ...
    if interactions == None and variables == None:
        ...
        md = smf.gee(formula, group_var, df_new, cov_struct=ind, family=fam)
        mdf = md.fit()
        ...
        for i, combo in enumerate(combos):
            ...
            df_results = pd.concat([df_results, (pd.DataFrame.from_dict(rhythm_params, orient='index')).T],
                                   ignore_index=True)
            osa=['kontrolna','testna']
            osa_raw=['izvorni podatki - kontrolna','izvorni podatki - testna']
            ...
            if plot:
                ...
                plotter.subplot_model(measurements_df[time_var], measurements_df[predict_var], measurements_df[time_var], measurements_df[predict_var], ax, plot_model=False,
                                      plot_measurements=True, color=colors[i], plot_measurements_with_color=True, raw_label=osa_raw[i])
    else:
        print("Interactions or variables can't be None.")
        return
...
def gee_cosopt(df, predict_var, time_var, group_var, variables=None,interactions=None,period=24, plot=True,plot_title='',save_to='',summary=False):
    df_results=pd.DataFrame()
    if plot:
        fig, ax = plt.subplots(1, 1, figsize=(12, 7))
...
def gee_arser(df, predict_var, time_var, group_var, n_periods=1,variables=None,interactions=None,plot=True,save_to='',plot_title='',est_periods=-1,summary=False):
    df_results=pd.DataFrame()
    if plot:
        fig, ax = plt.subplots(1, 1, figsize=(12, 7))

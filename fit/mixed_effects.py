...
def calculate_confidence_intervals_parameters(df, predict_var, time_var, group_var, variables=None, interactions=None,
                                              random_var="1", repetitions=20, period=24, n_components=1, plot=True,
                                              save_to=''):
    ...
    for i in range(0, repetitions):
        ...
        df_result = mixed_effects(sample, predict_var, time_var, group_var, interactions=interactions,
                                  variables=variables, random_var=random_var, n_components=n_components, period=period)
    ...
    for combo in combos:
        ...
        dict = {'parameter': helpers.tup2str(combo), 'amplitude': round(mean_amp, 2),
                'std_amplitude': round(std_amp, 2), 'amplitude_CIs': amplitude,
...
def mixed_effects(df, predict_var, time_var, group_var, random_var="1", variables=None, interactions=None,
                  n_components=1, period=24):
    df_results = pd.DataFrame()
...
def me_cosopt(df, predict_var, time_var, group_var, variables=None, interactions=None, period=24, random_var="1",
    ...
    for id in df[group_var].unique():
        ...
        temp_X_fit, temp_X_test, temp_X_fit_test, temp_X_fit_eval_params, temp_phase = methods.cosopt(
            temp[time_var].to_numpy(), temp[predict_var].to_numpy())
        phases.append(temp_phase)
        temp = pd.concat([temp.reset_index().drop(columns=['index']), pd.DataFrame(temp_X_fit, columns=columns)],
                         axis=1)
        if type(df_new) == int:
            df_new = temp.copy()
        else:
...
def me_arser(df, predict_var, time_var, group_var, n_periods=1, variables=None, interactions=None, random_var="1",
    ...
    for id in df[group_var].unique():
        ...
        if (type(est_periods) == int):
            temp_X_fit, temp_X_test, temp_X_fit_test, temp_X_fit_eval_params, temp_periods = methods.arser_eval_gee(
                temp[time_var].to_numpy(), temp[predict_var].to_numpy(), time_step=time_step, n_periods=n_periods)
            ...
            temp_X_fit, temp_X_test, temp_X_fit_test, temp_X_fit_eval_params = methods.arser(temp[time_var].to_numpy(),
                                                                                             temp_periods)

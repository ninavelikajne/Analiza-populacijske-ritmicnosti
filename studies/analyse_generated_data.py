...
names = ['asymmetric_oscillatory_2', 'symmetric_non_oscillatory', 'asymmetric_non_oscillatory', 'symmetric_oscillatory',
         'asymmetric_oscillatory_1']
for name in names:
    ...
    for ix, population_id in enumerate(df_data['population_id'].unique()):
        ...
        # 1_cosinor1
        df_cosinor1 = dproc.population_fit_cosinor1(df_pop, 24, save_to='', alpha=0.05, plot_on=False,
                                                    plot_individuals=False,
                                                    plot_measurements=False, plot_margins=False, color="blue",
                                                    hold_on=False,
                                                    plot_residuals=False, save_folder='')
        df_cosinor1 = df_cosinor1.iloc[0]
        tp, fp, tn, fn, classification = hlp.cosinor1_classify_rhythm(df_pop['rhythm'].iloc[0],
                                                                      df_cosinor1['p(amplitude)'])
        cosinor1_tp = cosinor1_tp + tp
        cosinor1_fp = cosinor1_fp + fp
        cosinor1_tn = cosinor1_tn + tn
        cosinor1_fn = cosinor1_fn + fn
        separeated = {'method': 'cosinor_1', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      'time_span': b['time_span'], 'num_of_period': b['num_of_period'], 'step': b['step'],
                      'replicates': b['replicates'], 'noise': b['noise'], 'population_id': population_id,
                      'repetition': repetition, 'amplitude': df_cosinor1['amplitude'],
                      'acrophase': df_cosinor1['acrophase'], 'mesor': df_cosinor1['mesor'],
                      'classification': classification, 'Y_test': np.array2string(df_cosinor1['Y_test'])}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosinor (n_comp=1)
        df_cosinor11, _ = dproc.fit_population(df_pop, 'cosinor', n_components=1, fig_name=str(population_id) + '_1',
                                               plot_population=False, plot_individuals=False)
        ...
        separeated = {'method': 'cosinor1', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      'time_span': b['time_span'], 'num_of_period': b['num_of_period'], 'step': b['step'],
                      'replicates': b['replicates'], 'noise': b['noise'], 'population_id': population_id,
                      'repetition': repetition, 'amplitude': df_cosinor11['amplitude'],
                      'acrophase': df_cosinor11['acrophase'], 'mesor': df_cosinor11['mesor'],
                      'classification': classification, 'Y_test': np.array2string(df_cosinor11['Y_test'])}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosinor2
        df_cosinor2, _ = dproc.fit_population(df_pop, 'cosinor', n_components=2, fig_name=str(population_id) + '_2',
                                              plot_population=False, plot_individuals=False)
        ...
        separeated = {'method': 'cosinor2', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      'time_span': b['time_span'], 'num_of_period': b['num_of_period'], 'step': b['step'],
                      'replicates': b['replicates'], 'noise': b['noise'], 'population_id': population_id,
                      'repetition': repetition, 'amplitude': df_cosinor2['amplitude'],
                      'acrophase': df_cosinor2['acrophase'], 'mesor': df_cosinor2['mesor'],
                      'classification': classification, 'Y_test': np.array2string(df_cosinor2['Y_test'])}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosinor3
        df_cosinor3, _ = dproc.fit_population(df_pop, 'cosinor', n_components=3, fig_name=str(population_id) + '_3',
                                              plot_population=False, plot_individuals=False)
        ...
        separeated = {'method': 'cosinor3', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      'time_span': b['time_span'], 'num_of_period': b['num_of_period'], 'step': b['step'],
                      'replicates': b['replicates'], 'noise': b['noise'], 'population_id': population_id,
                      'repetition': repetition, 'amplitude': df_cosinor3['amplitude'],
                      'acrophase': df_cosinor3['acrophase'], 'mesor': df_cosinor3['mesor'],
                      'classification': classification, 'Y_test': np.array2string(df_cosinor3['Y_test'])}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosopt
        df_cosopt, _ = dproc.fit_population(df_pop, 'cosopt', fig_name=str(population_id), plot_population=False,
                                            plot_individuals=False)
        ...
        separeated = {'method': 'cosopt', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      'time_span': b['time_span'], 'num_of_period': b['num_of_period'], 'step': b['step'],
                      'replicates': b['replicates'], 'noise': b['noise'], 'population_id': population_id,
                      'repetition': repetition, 'amplitude': df_cosopt['amplitude'],
                      'acrophase': df_cosopt['acrophase'], 'mesor': df_cosopt['mesor'],
                      'classification': classification, 'Y_test': np.array2string(df_cosopt['Y_test'])}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        try:
            df_arser1, df_periods = dproc.fit_population(df_pop, 'arser', n_periods=1, time_step=time_step,
                                                         fig_name=str(population_id) + '_1', plot_population=False,
                                                         plot_individuals=False, df_periods=df_periods,
            ...
            separeated = {'method': 'arser1', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                          'time_span': b['time_span'],
                          'num_of_period': b['num_of_period'], 'step': b['step'], 'replicates': b['replicates'],
                          'noise': b['noise'], 'population_id': population_id, 'repetition': repetition,
                          'amplitude': df_arser1['amplitude'],
        ...
        # arser2
        try:
            df_arser2, df_periods = dproc.fit_population(df_pop, 'arser', n_periods=2, time_step=time_step,
                                                         fig_name=str(population_id) + '_2', plot_population=False,
                                                         plot_individuals=False, df_periods=df_periods,
                                                         pop_id=population_id)
            ...
            separeated = {'method': 'arser2', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                          'time_span': b['time_span'],
                          'num_of_period': b['num_of_period'], 'step': b['step'], 'replicates': b['replicates'],
                          'noise': b['noise'], 'population_id': population_id, 'repetition': repetition,
                          'amplitude': df_arser2['amplitude'],
        ...
        # arser3
        try:
            df_arser3, df_periods = dproc.fit_population(df_pop, 'arser', n_periods=3, time_step=time_step,
                                                         fig_name=str(population_id) + '_3', plot_population=False,
                                                         plot_individuals=False, df_periods=df_periods,
                                                         pop_id=population_id)
            ...
            separeated = {'method': 'arser3', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                          'time_span': b['time_span'],
                          'num_of_period': b['num_of_period'], 'step': b['step'], 'replicates': b['replicates'],
                          'noise': b['noise'], 'population_id': population_id, 'repetition': repetition,
                          'amplitude': df_arser3['amplitude'],
    ...
    cosinor1_result = {'method': 'cosinor_1', 'data': name, 'tp': cosinor1_tp, 'fp': cosinor1_fp, 'tn': cosinor1_tn,
                       'fn': cosinor1_fn}
    cosinor11_result = {'method': 'cosinor1', 'data': name, 'tp': cosinor11_tp, 'fp': cosinor11_fp, 'tn': cosinor11_tn,
                        'fn': cosinor11_fn}
    cosinor2_result = {'method': 'cosinor2', 'data': name, 'tp': cosinor2_tp, 'fp': cosinor2_fp, 'tn': cosinor2_tn,
                       'fn': cosinor2_fn}
    cosinor3_result = {'method': 'cosinor3', 'data': name, 'tp': cosinor3_tp, 'fp': cosinor3_fp, 'tn': cosinor3_tn,
                       'fn': cosinor3_fn}
    cosopt_result = {'method': 'cosopt', 'data': name, 'tp': cosopt_tp, 'fp': cosopt_fp, 'tn': cosopt_tn,
                     'fn': cosopt_fn}
    arser1_result = {'method': 'arser1', 'data': name, 'tp': arser1_tp, 'fp': arser1_fp, 'tn': arser1_tn,
                     'fn': arser1_fn}
    arser2_result = {'method': 'arser2', 'data': name, 'tp': arser2_tp, 'fp': arser2_fp, 'tn': arser2_tn,
                     'fn': arser2_fn}
    arser3_result = {'method': 'arser3', 'data': name, 'tp': arser3_tp, 'fp': arser3_fp, 'tn': arser3_tn,
                     'fn': arser3_fn}

    df_results = pd.concat([df_results, (pd.DataFrame.from_dict(cosinor1_result, orient='index')).T], ignore_index=True)
    df_results = pd.concat([df_results, (pd.DataFrame.from_dict(cosinor11_result, orient='index')).T],
                           ignore_index=True)

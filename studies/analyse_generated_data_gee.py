...
from fit import helpers as hlp, gee
...
names = ['symmetric_non_oscillatory', 'asymmetric_non_oscillatory', 'symmetric_oscillatory', 'asymmetric_oscillatory_1',
         'asymmetric_oscillatory_2']
for name in names:
    ...
    for ix, population_id in enumerate(df_data['population_id'].unique()):
        ...
        # cosinor (n_comp=1)
        df_cosinor11 = gee.gee_cosinor(df_pop, 'Y', 'X', 'id', n_components=1, plot=False)
        ...
        separeated = {'method': 'cosinor1', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      ...
                      'classification': classification, 'Y_test': np.array2string(df_cosinor11['Y_test'].to_numpy())}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosinor2
        df_cosinor2 = gee.gee_cosinor(df_pop, 'Y', 'X', 'id', n_components=2, plot=False)
        ...
        separeated = {'method': 'cosinor2', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      ...
                      'classification': classification, 'Y_test': np.array2string(df_cosinor2['Y_test'].to_numpy())}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosinor3
        df_cosinor3 = gee.gee_cosinor(df_pop, 'Y', 'X', 'id', n_components=3, plot=False)
        ...
        separeated = {'method': 'cosinor3', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      ...
                      'classification': classification, 'Y_test': np.array2string(df_cosinor3['Y_test'].to_numpy())}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        # cosopt
        df_cosopt = gee.gee_cosopt(df_pop, 'Y', 'X', 'id', plot=False)
        ...
        separeated = {'method': 'cosopt', 'data_name': name, 'data_params': a, 'changing_param': changing_param,
                      ...
                      'classification': classification, 'Y_test': np.array2string(df_cosopt['Y_test'].to_numpy())}
        df_separated = pd.concat([df_separated, (pd.DataFrame.from_dict(separeated, orient='index')).T],
                                 ignore_index=True)

        try:
            df_arser1 = gee.gee_arser(df_pop, 'Y', 'X', 'id', n_periods=1, plot=False, est_periods=df_periods_pop)
        ...
        # arser2
        try:
            df_arser2 = gee.gee_arser(df_pop, 'Y', 'X', 'id', n_periods=2, plot=False, est_periods=df_periods_pop)
        ...
        # arser3
        try:
            df_arser3 = gee.gee_arser(df_pop, 'Y', 'X', 'id', n_periods=3, plot=False, est_periods=df_periods_pop)
...
df_results.to_csv('./results/classification_gee.csv', index=False)
df_separated.to_csv('./results/sep_classification_gee.csv', index=False)
